...
file_list <- list.files(path = input_dir, pattern = "\\_songs.txt$", full.names = TRUE)
...
# Read file
notes_selections <- read.table(file_path, header = TRUE, sep = "\t", check.names = F)
# Compute metrics
...
write.table(songs_selection,output_path, sep = "\t", row.names = FALSE, quote = FALSE)
